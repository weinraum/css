...
/* $a ist der jeweilige DTO aus buildDto(...); $a['agg'] wird nur gesetzt, wenn:
die Section type === 'article_full' ist und
contentService->byId($cid) ein Aggregate liefert und
status == 1 im wr_content-Datensatz ist.
Wenn eine dieser Bedingungen nicht erfüllt ist (falsche ID, Status ≠ 1, Exception im try/catch), bleibt $a['agg'] leer – dann greift dein Fallback („Teaser ohne Bild“).
* 
*/
        if ($type === 'article_full' && $cid > 0) {
            try {
                ...
                if ((int)($agg->content['status'] ?? 0) === 1) {
                    $dto['agg'] = $agg;  // <-- HIER wird $a['agg'] gesetzt
                }
            } catch (\Throwable) {}
        }
