...
function imagesTemplate($data = NULL, $_index = NULL, $_length = NULL) {
$html = '';
$ids = array();
$prefix = '';
if ($data === NULL) $prefix = '_';

$var = 'file';
if (isset($data[$var][$_index])) {
$html .= '<div class="control-group">'.PHP_EOL;
$html .= '  <label class="control-label">&nbsp;</label>'.PHP_EOL;
$html .= '  <div class="controls">'.PHP_EOL;
$html .= '      <img src="/_image/'.$data[$var][$_index]['filename'].'" alt="" />'.PHP_EOL;
$html .= '  </div>'.PHP_EOL;
$html .= '</div>'.PHP_EOL;
}
else {
$html .= '<div class="control-group">'.PHP_EOL;
$html .= '  <label class="control-label">'.sprintf(lang('german_lang.themengarten_content_gallery_'.$var), ($data===NULL?'':$_index+1)).'</label>'.PHP_EOL;
$html .= '  <div class="controls">'.PHP_EOL;
$html .= '      <div class="fileupload fileupload-new" data-provides="fileupload">'.PHP_EOL;
$html .= '          <div class="input-append">'.PHP_EOL;
$html .= '              <input type="hidden" name="_'.$var.'['.$_index.']" value="'.$data[$var][$_index]['id'].'" />'.PHP_EOL;
...
if ($data[$var][$_index]['id']) $ids[] = $data[$var][$_index]['id'];

$var = 'headline';
$html .= '<div class="control-group">'.PHP_EOL;
$html .= '  <label class="control-label">'.sprintf(lang('german_lang.themengarten_content_fancy_'.$var), ($data===NULL?'':$_index+1)).'</label>'.PHP_EOL;
$html .= '  <div class="controls">'.PHP_EOL;
$html .= '      <input type="hidden" name="_'.$prefix.$var.'['.$_index.']" value="'.$data[$var][$_index]['id'].'" />'.PHP_EOL;
$html .= '      <input type="text" name="'.$prefix.$var.'['.$_index.']" class="input-block-level" value="'.htmlspecialchars($data[$var][$_index]['value']).'" />'.PHP_EOL;
$html .= '  </div>'.PHP_EOL;
$html .= '</div>'.PHP_EOL;
if ($data[$var][$_index]['id']) $ids[] = $data[$var][$_index]['id'];
...
$html .= '  <div class="controls">'.PHP_EOL;
$html .= '      <input type="hidden" name="_'.$prefix.$var.'['.$_index.']" value="'.$data[$var][$_index]['id'].'" />'.PHP_EOL;
$html .= '      <textarea name="'.$prefix.$var.'['.$_index.']" class="input-block-level">'.htmlspecialchars($data[$var][$_index]['value']).'</textarea>'.PHP_EOL;
$html .= '  </div>'.PHP_EOL;
$html .= '</div>'.PHP_EOL;
if ($data[$var][$_index]['id']) $ids[] = $data[$var][$_index]['id'];
...
<h2><?php echo lang('german_lang.themengarten_content_'.$type); ?> - <?php echo lang('german_lang.themengarten_content_block'); ?></h2>
...
$var1 = 'headline'; $t_data[$var1] = (!is_array($POST[$var1])?array($POST[$var1]):$POST[$var1]);
$var1 = 'text'; $t_data[$var1] = (!is_array($POST[$var1])?array($POST[$var1]):$POST[$var1]);
$var1 = 'file'; $t_data[$var1] = (!is_array($POST[$var1])?array($POST[$var1]):$POST[$var1]);
$var1 = 'weiID'; $t_data[$var1] = (!is_array($POST[$var1])?array($POST[$var1]):$POST[$var1]);
$var1 = 'packID'; $t_data[$var1] = (!is_array($POST[$var1])?array($POST[$var1]):$POST[$var1]);
$var1 = 'link'; $t_data[$var1] = (!is_array($POST[$var1])?array($POST[$var1]):$POST[$var1]);


foreach($POST[$var] as $k => $file) {
$t_data['file'][$k]['filename'] = 'prev-804_'.$POST['content_id'].'_'.$POST['slice_id'].'_'.$t_data['file'][$k]['id'].'__'.$t_data['file'][$k]['value'];
echo imagesTemplate($t_data, $k, sizeof($POST[$var]));
}
}
?>
